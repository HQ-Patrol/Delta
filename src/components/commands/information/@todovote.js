	...
	run: async (client, message, args) => {
		...
		if(!voted) {
          ...
          "\n<:MysteryBox1:855561382896468021> **For every Vote, you can CLAIM Mystery Boxes/Eggs and 1000** <a:Coins:775714101564276756> ")
			...
			var weekly = await Weekly.findOne({ id: message.author.id });
			if(!weekly) { weekly = new Weekly({
			...
			if(weekly.voteReset > Date.now()) return message.channel.send({ embeds: [
			...
			var data = await Eco.findOne({ id: message.author.id });
			if(!data) {
				data = new Eco({
					id: message.author.id,
					coins: 0,
