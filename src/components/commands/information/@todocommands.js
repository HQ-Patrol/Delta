...
async function getAll(client, message) {
    ...
    let guild = await Guilds.findOne({ _id: message.guild.id }).catch(err => console.log(err));
    ...
    for (i = 0; i < Math.ceil(client.categories.length/5); i++) {
        ...
        for (j = i*5; j < (i*5)+5; j++) {
            let entries = Object.entries(cattocommands)[j]
            //console.log(entries)
...
async function getCMD(client,message,input) {
    const embed = new MessageEmbed()
    .setColor("RANDOM")
    let cmd = client.commands.get(input.toLowerCase()) || client.commands.find(cmd => cmd.aliases && cmd.aliases.includes(input.toLowerCase()))
