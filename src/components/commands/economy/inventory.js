...
module.exports.slash = {
  data: new Discord.SlashCommandBuilder()
    .setName("inventory")
    .setDescription("View yours or someone elses inventory")
  ...
  async handler(client, interaction) {
    ...
    if (target.bot) return interaction.sendError("You didn't possibly just tag a bot 😐");
      ...
      return interaction.reply({
        embeds: [
          new Discord.EmbedBuilder()
            ...
            .setAuthor({ name: `${target.username}'s Inventory 🎒`, iconURL: target.displayAvatarURL() })
            .setDescription("**[Inventory is empty!]**")
            .addDescription("Visit the shop by typing `/shop` 🛒"),
        ],
      });
    }

    const chunks = client.utils.chunk(person.items.filter((i) => i?.count > 0), 8);
    let page = 1;
    const totalPages = chunks.length;
    const tip = client.utils.footer();
